...
public class TaskManager {
    ...
    public int PlayTask(Bee ghost, Player player){
        ...
        taskId[0] = Bukkit.getScheduler().scheduleSyncRepeatingTask(B_s_GhostGrave.getPlugin(), () -> {
            if (player != null && ghost != null){ // Check if it's valid to do the task
                ...
                //Check if the dimension of the player is the same as the ghosts (needed for distance calculation)
                if (ghost.getWorld() == player.getWorld())
                {
            ...
            {
                B_s_GhostGrave.getPlugin().getLogger().warning("Ghost not working properly at: "+ghost.getLocation());
            }
        }, 0L, 2L);
